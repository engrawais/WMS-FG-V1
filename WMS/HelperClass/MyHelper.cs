...
namespace WMS.HelperClass
{
    ...
    public class ManualDailyRequest
    {
        public void SaveManualRequest(int empid, DateTime? dts, DateTime? dte)
        {
            using (var db = new TAS2013Entities())
            {
                AttProcessorScheduler atp = new AttProcessorScheduler();
                atp.WhenToProcess=DateTime.Today;
                atp.PeriodTag = "D";
                atp.EmpID=empid;
                atp.EmpNo=atp.EmpID.ToString();
                atp.DateFrom = (DateTime)dts;
                atp.DateTo = (DateTime)dte;
                atp.CompanyID=1;
                atp.CreatedDate=DateTime.Now;
                atp.LocationID=1;
                atp.ProcessCat=false;
                atp.ProcessingDone=false;
                atp.UserID=25;
                db.AttProcessorSchedulers.Add(atp);
                db.SaveChanges();
            }
        }
        public void SaveManualRequest(int empid, DateTime dts, DateTime dte)

    ...
    public class LeaveController
    {
        ...
        public bool CheckLeaveBalance(LvApplication _lvapp)
        {
            ...
            using (var ctx = new TAS2013Entities())
            {
                ...
                string empLvType = _lvapp.EmpID.ToString() + _lvapp.LvType+ _lvapp.FromDate.Year.ToString();;
                ...
                for (int i = 0; i < lvappl.NoOfDays; i++)
                {
                    ...
                    using (var context = new TAS2013Entities())
                    {
                        if (context.AttDatas.Where(aa => aa.EmpDate == _EmpDate).Count() > 0)
                        {
                            ...
                            LvType lvtype = context.LvTypes.First(aa => aa.LvType1 == lvappl.LvType);
        ...
        public void BalanceLeaves(LvApplication lvappl)
        {
            ...
            {
                List<LvConsumed> _lvConsumed = new List<LvConsumed>();
                string empLvType = lvappl.EmpID.ToString() + lvappl.LvType+lvappl.FromDate.Year.ToString();
                _lvConsumed = ctx.LvConsumeds.Where(aa => aa.EmpLvTypeYear == empLvType).ToList();
        ...
        public bool CheckHalfLeaveBalance(LvApplication lvapplication)
        {
            ...
            using (var ctx = new TAS2013Entities())
            {
                string empLvType = lvapplication.EmpID.ToString() + lvapplication.LvType+lvapplication.FromDate.Year.ToString();
                _lvConsumed = ctx.LvConsumeds.Where(aa => aa.EmpLvTypeYear == empLvType).ToList();
                if (_lvConsumed.Count > 0)
                {
        ...
        public bool DeleteFromLVData(LvApplication lvappl)
        {
            bool check=false;
        ...
        public bool HasLeaveQuota(int empID, string lvType,DateTime dt)
        {
            ...
            using (var ctx = new TAS2013Entities())
            {
                ...
                if (LeaveType.UpdateBalance == true)
                {
                    ...
                    string empLvType = empID.ToString() + lvType + dt.Year.ToString();

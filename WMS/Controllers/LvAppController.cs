...
namespace WMS.Controllers
{
    ...
    public class LvAppController : Controller
    {
        ...
        public ActionResult Index(string sortOrder, string searchString, string currentFilter, int? page)
        {
            ...
            DateTime dt2 = new DateTime(dt1.Year-1, 1, 1);
            ...
            return View(lvapplications.OrderByDescending(aa=>aa.LvDate).ToPagedList(pageNumber, pageSize));
        }
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            {
                if (LvProcessController.DeleteFromLVData(lvapplication))
                {
            ...
            {
                if (LvProcessController.DeleteHLFromLVData(lvapplication))
                {
            ...
            ManualDailyRequest mdr = new ManualDailyRequest();
            mdr.SaveManualRequest(lvapplication.EmpID, lvapplication.FromDate, lvapplication.ToDate);
        ...
        public ActionResult GetEmpInfoForLeaves(string ID)
        {
            string[] words = ID.Split('w');
            int companyID = Convert.ToInt16(words[1]);
            string EmpNo = words[0];
            string Year = words[2];
            List<Emp> emp = db.Emps.Where(aa => aa.CompanyID == companyID && aa.EmpNo == EmpNo&&aa.Status==true).ToList();
            if (emp.Count > 0)
            {

...
namespace WMS.Controllers
{
    public class AttProSingleController : Controller
    {
        ...
        public ViewResult Index(string sortOrder, string searchString, string currentFilter, int? page)
        {
            ...
            List<AttProcessorScheduler> attprocess = context.AttProcessorSchedulers.Where(aa => aa.ProcessingDone == false).OrderByDescending(aa => aa.CreatedDate).ToList();
            switch (sortOrder)
            {
        ...
        public ActionResult Create(AttProcessorScheduler attprocessor)
        {
            ...
            if ((attprocessor.DateTo - attprocessor.DateFrom).Days > 40)
                ModelState.AddModelError("DateTo", "Date Difference must be between 40 days!");
